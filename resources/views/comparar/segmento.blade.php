...
<div class="container mt-5 d-flex justify-content-center">
    <div class="card shadow-lg p-5 rounded-lg text-center" style="max-width: 500px;">
        ...
        <form action="{{ route('comparar.resultado') }}" method="GET">
            {{-- Tipo para activar el comparador dinámico de segmento --}}
            <input type="hidden" name="tipo" value="segmento">

            <div class="mb-4">
                <label for="auto1" class="form-label fw-bold">Selecciona el primer auto:</label>
                <select name="auto1" class="form-select rounded-sm shadow-sm" required>
            ...
            <div class="mb-4">
                ...
                <select name="auto2" class="form-select rounded-sm shadow-sm" required>
                    @foreach ($autos as $auto)
                        <option value="{{ $auto->id }}">{{ $auto->marca }} {{ $auto->modelo }} ({{ $auto->anio }})</option>
                    @endforeach
                </select>
